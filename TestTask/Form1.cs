...
namespace TestTask
{
    ...
    {
        Сalculation _сalculation;
        FileWork _fileWork;
        public Form1()
        {
            _сalculation = new Сalculation();
            _fileWork = new FileWork();
            InitializeComponent();
            var bmp = _сalculation.Main();
        ...
        private void pictureBox2_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            var g = Graphics.FromImage(_сalculation.bmp3);
            var pen = new Pen(Color.Black, 1f);
            g.DrawLine(pen, _сalculation.start, finish);
            g.Save();
            drawing = false;
            g.Dispose();
            ...
            _сalculation.coordsim.Add(xy);
        }

        private void pictureBox2_MouseDown(object sender, MouseEventArgs e)
        {
            _сalculation.start = new Point(e.X, e.Y);
            _сalculation.orig = _сalculation.bmp3;
            ...
            if (_сalculation.coordsim.Count == 0)
            {
                PointF xy = new PointF
                {
                    X = rect.X - (pictureBox2.Width / 2),
        ...
        private void pictureBox2_MouseMove(object sender, MouseEventArgs e)
        {
            if (!drawing) return;
            var finish = new Point(e.X, e.Y);
            _сalculation.bmp2 = new Bitmap(_сalculation.bmp3);
            pictureBox2.Image = _сalculation.bmp2;
            var pen = new Pen(Color.Black, 1f);
            var g = Graphics.FromImage(_сalculation.bmp2);
            g.DrawLine(pen, _сalculation.start, finish);
            g.Dispose();
            rect.X = e.X - deltaX;
            rect.Y = e.Y - deltaY;
